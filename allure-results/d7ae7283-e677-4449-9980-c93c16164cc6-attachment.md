...
- Name: tests\e2e\clients\TNDI\newPatient.spec.js >> Symptoms autocomplete >> TC-INT-08 — invalid search term shows no options
- Location: tests\e2e\shared\intake.cases.js:91:9
...
> 95  |             ).toHaveCount(0, { timeout: 5_000 });
  ...
  191 |     });
  192 | 
  193 |     test.describe('Continue button', () => {
  194 | 
  195 |         test('TC-INT-H06 — Continue is visible and enabled', async ({ intakePage }) => {
```