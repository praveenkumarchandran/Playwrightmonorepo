...
- Name: tests\e2e\clients\CVD\newPatient.spec.js >> Invalid input format validation >> TC-PI-VAL-05 — form with valid data but too-short phone does not submit
...
Error: [CVD] patientInfoPage fixture failed.
  ...
  Original error: page.goto: net::ERR_ABORTED at todo: paste CVD URL here
Call log:
  - navigating to "todo: paste CVD URL here", waiting until "networkidle"
...
```ts
  ...
  76  |             } catch (e) {
  77  |                 if (e.message.startsWith('NO_SLOTS_AVAILABLE')) {
  ...
  124 |                 if (e.message.startsWith('NO_SLOTS_AVAILABLE')) {
  ...
  147 |                 if (e.message.startsWith('NO_SLOTS_AVAILABLE')) {
...
> 151 |                 throw new Error(`[${clientKey}] patientInfoPage fixture failed.\n  Flow: ${flow.join(' → ')}\n  ${e.message}`);
  ...
  163 |             } catch (e) {
  164 |                 if (e.message.startsWith('NO_SLOTS_AVAILABLE')) {
