...
Call log:
  - waiting for locator('button:has-text("2:30"),\n     [role="button"]:has-text("2:30"),\n     div[class*="slot"]:has-text("2:30"),\n     span:has-text("2:30 PM"),\n     td:has-text("2:30")').first() to be visible
...
> 187 |     await slotBtn.waitFor({ timeout: 15000 });
  ...
  266 |             } else {
  267 |                 try {
  268 |                     await selectDropdown(page, sel, 'Self Pay');
  269 |                     break;
  270 |                 } catch {
  271 |                     continue;
  272 |                 }
  ...
  278 | 
  279 |     // Click "Manually Enter Details"
  280 |     await clickButton(page, 'Manually Enter Details');
  281 |     await page.waitForTimeout(1000);
  282 | 
  283 |     // Insurance field → select "Other"
  284 |     const insuranceFieldSelectors = [
  285 |         '[class*="insurance"] select:nth-of-type(2)',
  286 |         'select[name*="plan" i]',
  287 |         '[class*="select"] [class*="control"]',
```