...
- Location: tests\e2e\shared\patientInfo.cases.js:236:9
...
```ts
  140 |     //     test('TC15 — dropdown closes after selection', async ({ patientPage }) => {
  141 |     //         await patientPage.selectGender('Male');
  142 |     //         await expect(patientPage.genderTrigger).toHaveAttribute('aria-expanded', 'false');
  143 |     //     });
  144 | 
  145 |     // });
  146 | 
