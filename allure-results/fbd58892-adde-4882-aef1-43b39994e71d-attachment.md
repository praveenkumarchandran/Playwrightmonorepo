...
- Location: tests\e2e\shared\patientInfo.cases.js:287:9
...
- paragraph: 9:20 AM, Thu Jun 11, 2026
- heading "Appointment Type" [level=6]
- paragraph: Skin Problem
- text: First Name *
- textbox "First Name *": Mary-Jane
- text: Last Name *
- textbox "Last Name *": O'Brien
- text: Date of Birth *
- textbox "Date of Birth *":
  - /placeholder: MM/DD/YYYY
  - text: 01/15/1940
- button "Choose date, selected date is Jan 15, 1940"
- combobox "Male"
- text: Email *
...
- textbox "Address2 (Optional)": Suite 400
- text: City *
...
```ts
  191 |             test('TC-PI-22 — State filters options by typed text', async ({ patientPage }) => {
  192 |                 await patientPage.stateInput.click();
  193 |                 await patientPage.stateInput.clear();
  194 |                 await patientPage.stateInput.pressSequentially('Mich', { delay: 50 });
  195 |                 await expect(
  196 |                     patientPage.page.locator('[role="option"]').filter({ hasText: 'MI-Michigan' }).first()
  197 |                 ).toBeVisible({ timeout: 10_000 });
  198 |             });
  199 | 
  200 |             test('TC-PI-23 — invalid state search shows no matching options', async ({ patientPage }) => {
  201 |                 await patientPage.stateInput.click();
  202 |                 await patientPage.stateInput.clear();
  203 |                 await patientPage.stateInput.pressSequentially('zzzznotastate', { delay: 50 });
  204 |                 // No [role="option"] items should appear in the listbox
  205 |                 await expect(
  206 |                     patientPage.page.locator('[role="option"]')
